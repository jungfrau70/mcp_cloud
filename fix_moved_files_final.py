#!/usr/bin/env python3
"""
파일 이동으로 인한 링크 수정 최종 도구
실제 파일 이동 내역에 맞춰 링크 업데이트
"""
...
class MovedFilesFinalFixer:
    ...
    def find_moved_file_path(self, broken_path: str) -> str:
        """이동된 파일의 올바른 경로 찾기"""
    ...
    def fix_links_in_content(self, content: str) -> Tuple[str, int]:
        ...
        for text, url, line_num in links:
            ...
            # 이동된 파일의 올바른 경로 찾기
            correct_path = self.find_moved_file_path(url)
    ...
    def generate_broken_links_report(self):
        ...
        report = {
            "timestamp": "2025-09-18T21:20:00",
            "total_broken_links": len(self.broken_links),
            "broken_links": self.broken_links
        }
        
        with open("moved_files_broken_links_report.json", "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        
        print(f"\n📋 깨진 링크 보고서 생성: moved_files_broken_links_report.json")
    
    def run_final_fix(self):
        """최종 수정 실행"""
        print("🔧 파일 이동으로 인한 링크 수정 최종 실행...")
        print("=" * 60)
...
def main():
    """메인 함수"""
    base_path = Path.cwd()
    fixer = MovedFilesFinalFixer(base_path)
    fixer.run_final_fix()
