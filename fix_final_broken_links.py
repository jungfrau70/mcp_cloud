...
class FinalBrokenLinksFixer:
    ...
    def fix_external_links(self, url: str) -> str:
        """외부 링크 수정"""
        # https:/ → https:// 수정
    ...
    def find_alternative_file(self, broken_path: str) -> str:
        ...
        # 5. 존재하지 않는 파일의 경우 해당 디렉토리로 링크
        parent_dir = str(Path(normalized_path).parent)
        if parent_dir in self.actual_files:
            return parent_dir
    ...
    def fix_links_in_content(self, content: str) -> Tuple[str, int]:
        ...
        for text, url, line_num in links:
            ...
            if url.startswith(('http://', 'https://')):
                fixed_url = self.fix_external_links(url)
                if fixed_url != url:
                    old_link = f"[{text}]({url})"
                    new_link = f"[{text}]({fixed_url})"
                    content = content.replace(old_link, new_link)
                    fixes_count += 1
                    print(f"    🔧 외부 링크 수정: {url} → {fixed_url}")
                continue
            
            # 내부 링크 처리
            if self.is_internal_link(url):
                # 올바른 파일 경로 찾기
                correct_path = self.find_alternative_file(url)
                if correct_path and correct_path != url:
                    ...
                    content = content.replace(old_link, new_link)
                    fixes_count += 1
                    print(f"    🔧 내부 링크 수정: {url} → {correct_path}")
...
def main():
    """메인 함수"""
    base_path = Path.cwd()
    fixer = FinalBrokenLinksFixer(base_path)
    fixer.run_final_fix()
