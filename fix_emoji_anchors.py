#!/usr/bin/env python3
"""
이모지 포함 앵커 링크 자동 수정 도구
VS Code 마크다운 미리보기 표준에 맞춰 이모지를 포함한 앵커 링크를 수정합니다.
"""
...
def extract_headings_with_emoji(content):
    """마크다운에서 이모지 포함 헤딩 추출"""
    headings = []
    lines = content.split('\n')
...
def fix_emoji_anchor_links(content):
    """이모지 포함 앵커 링크 수정"""
    # 헤딩 추출
    headings = extract_headings_with_emoji(content)
    ...
    def replace_anchor_link(match):
        ...
        # VS Code 표준 이모지 포함 앵커 ID 생성
        correct_anchor = generate_vscode_emoji_anchor_id(link_text)
...
def process_file(file_path):
    ...
    try:
        ...
        # 앵커 링크 수정
        fixed_content = fix_emoji_anchor_links(content)
