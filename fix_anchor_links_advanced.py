...
"""
고급 앵커 링크 자동 수정 도구
남은 15개 문제를 해결하기 위한 개선된 매칭 로직을 적용합니다.
"""
...
class AdvancedAnchorLinkFixer:
    ...
    def normalize_special_characters(self, text: str) -> str:
        """특수문자 정규화"""
        # 괄호 내 영어 처리: (Self-Healing) -> self-healing
        text = re.sub(r'\(([^)]+)\)', r'-\1', text)
        
        # 슬래시 처리: CI/CD -> ci-cd, Git/GitHub -> git-github
    ...
    def find_matching_heading_advanced(self, doc_content: str, target_anchor: str, link_text: str) -> Tuple[str, int]:
        """고급 매칭 로직으로 헤딩 찾기"""
        ...
        for i, line in enumerate(lines):
            ...
            if match and match.group(2):
                ...
                if clean_text:
                    ...
                    # 3. 특수문자 정규화 후 매칭
                    normalized_text = self.normalize_special_characters(clean_text)
                    normalized_id = self.normalize_anchor_id(normalized_text)
                    if normalized_id == target_anchor:
                        return clean_text, i + 1
                    
                    # 4. 앞에 하이픈 제거 후 매칭
                    if target_anchor.startswith('-'):
                        without_prefix = target_anchor[1:]
                        if exact_id == without_prefix:
                            return clean_text, i + 1
                        if emoji_removed_id == without_prefix:
                            return clean_text, i + 1
                        if normalized_id == without_prefix:
                            return clean_text, i + 1
                    
                    # 5. CI/CD 특별 처리
                    if 'cicd' in target_anchor.lower():
                        # CI/CD -> ci-cd 변환 시도
                        cicd_variants = [
                            clean_text.replace('CI/CD', 'ci-cd'),
                            clean_text.replace('ci/cd', 'ci-cd'),
                            clean_text.replace('CI / CD', 'ci-cd'),
                            clean_text.replace('ci / cd', 'ci-cd')
                        ]
                        for variant in cicd_variants:
                            variant_id = self.normalize_anchor_id(variant)
                            if variant_id == target_anchor:
                                return clean_text, i + 1
                    
                    # 6. GitHub 특별 처리
                    if 'gitgithub' in target_anchor.lower():
                        # Git/GitHub -> git-github 변환 시도
                        github_variants = [
                            clean_text.replace('Git/GitHub', 'git-github'),
                            clean_text.replace('git/github', 'git-github'),
                            clean_text.replace('Git / GitHub', 'git-github'),
                            clean_text.replace('git / github', 'git-github')
                        ]
                        for variant in github_variants:
                            variant_id = self.normalize_anchor_id(variant)
                            if variant_id == target_anchor:
                                return clean_text, i + 1
                    
                    # 7. GCP Cloud Run 특별 처리
                    if 'gcp-cloud-run' in target_anchor.lower():
                        # GCP Cloud Run -> gcp-cloud-run 변환 시도
                        gcp_variants = [
                            clean_text.replace('GCP Cloud Run', 'gcp-cloud-run'),
                            clean_text.replace('gcp cloud run', 'gcp-cloud-run'),
                            clean_text.replace('GCP cloud run', 'gcp-cloud-run')
                        ]
                        for variant in gcp_variants:
                            variant_id = self.normalize_anchor_id(variant)
                            if variant_id == target_anchor:
                                return clean_text, i + 1
                    
                    # 8. Self-Healing 특별 처리
                    if 'self-healing' in target_anchor.lower():
                        # 자가 치유(Self-Healing) -> 자가-치유-self-healing 변환 시도
                        healing_variants = [
                            clean_text.replace('(Self-Healing)', '-self-healing'),
                            clean_text.replace('(self-healing)', '-self-healing'),
                            clean_text.replace('Self-Healing', '-self-healing'),
                            clean_text.replace('self-healing', '-self-healing')
                        ]
                        for variant in healing_variants:
                            variant_id = self.normalize_anchor_id(variant)
                            if variant_id == target_anchor:
                                return clean_text, i + 1
                    ...
                    # 10. 링크 텍스트와 직접 비교
                    link_normalized = self.normalize_anchor_id(link_text)
                    if link_normalized == target_anchor:
                        return clean_text, i + 1
        
        return None, -1
    
    def fix_document_anchors_advanced(self, file_path: str, issues: List[Dict]) -> bool:
        """고급 매칭 로직으로 문서의 앵커 링크 수정"""
        ...
        for issue in issues:
            ...
            # 고급 매칭 로직으로 헤딩 찾기
            heading_text, heading_line = self.find_matching_heading_advanced(content, old_href, link_text)
    ...
    def fix_all_remaining_issues(self, report_path: str = "anchor_link_audit_report.json"):
        """남은 모든 문제 수정"""
        print("🔧 고급 앵커 링크 자동 수정 시작")
        print("=" * 50)
        ...
        for file_path, issues in report['issues_by_file'].items():
            ...
            success = self.fix_document_anchors_advanced(file_path, issues)
