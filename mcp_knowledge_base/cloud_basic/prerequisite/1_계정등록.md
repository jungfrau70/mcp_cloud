...
*   권장 권한 모델 단일 소스: `cloud_basic/cloud_basic/prerequisite/3_역할분담.md` "권장 권한 모델(요약) — 재무 vs IT 운영" 절을 기준으로 참조합니다.
...
*   루트 사용자 로그인: https://aws.amazon.com/ko/
...
*   예시 CLI
    ...
    \`\`\`  
    aws iam create-group --group-name CostManagers aws iam attach-group-policy --group-name CostManagers  
    \--policy-arn arn:aws:iam::aws:policy/job-function/Billing aws iam attach-group-policy --group-name CostManagers  
...
*   실행 예(사전 `aws configure` 또는 SSO 설정 필요)
    ...
    THRESHOLDS=80,100 SUBSCRIBERS=you@example.com bash cloud\_basic/automation/cli/budget/aws\_budget.sh
...
*   Windows 환경변수 예:
    ...
    setx AWS\_PROFILE default setx AWS\_REGION ap-northeast-2
...
1.  Google 계정 준비(또는 조직 계정) 후 콘솔 접속
...
*   실행 예:
    ...
    setx PROJECT\_ID setx BILLING\_ACCOUNT THRESHOLD\_PERCENT=80 BUDGET\_AMOUNT=10 bash cloud\_basic/automation/cli/budget/gcp\_budget.sh
...
*   실행 예:
    ...
    setx SUB\_ID THRESHOLD\_PERCENT=80 BUDGET\_AMOUNT=10 CONTACT\_EMAILS=you@example.com bash cloud\_basic/automation/cli/budget/azure\_budget.sh
...
*   사용 예(모듈 디렉토리에서 실행 전):
    ...
    source ../../guard.sh || . ../../guard.sh terraform init && terraform plan
    
    chmod +x cloud\_basic/automation/cli/aws/\*.sh AWS\_PROFILE=default AWS\_REGION=ap-northeast-2 bash cloud\_basic/automation/cli/aws/ch1\_iam.sh
...
*   Chapter1\_IAM.md 실습 진행
    
*   실습\_시나리오.md를 따라 VM/스토리지/네트워크 순으로 진행
