...
# Cloud Basic - 1일차: AWS & GCP 기초 서비스 이론 및 실습
...
## 📚 이론 학습
1. [🎯 학습 목표](#학습-목표)
2. [📖 클라우드 개념 및 계정 생성 이론](#클라우드-개념-및-계정-생성-이론)
3. [🔐 IAM 기초 이론](#iam-기초-이론)
4. [💻 가상머신 서비스 기초 이론](#가상머신-서비스-기초-이론)
5. [📦 스토리지 서비스 기초 이론](#스토리지-서비스-기초-이론)

## 🛠️ 실습 학습
6. [🔧 실습 환경 준비](#실습-환경-준비)
7. [🚀 클라우드 계정 생성 실습](#클라우드-계정-생성-실습)
8. [🔐 IAM 기초 실습](#iam-기초-실습)
9. [💻 가상머신 서비스 실습](#가상머신-서비스-실습)
10. [📦 스토리지 서비스 실습](#스토리지-서비스-실습)
...
<details>
<summary>📖 클라우드 컴퓨팅 기본 개념</summary>

#### 클라우드 컴퓨팅이란?
클라우드 컴퓨팅은 인터넷을 통해 컴퓨팅 리소스(서버, 스토리지, 데이터베이스, 네트워킹, 소프트웨어, 분석, 인텔리전스)를 제공하는 서비스입니다.

#### 클라우드의 주요 특징
- **온디맨드 셀프 서비스**: 사용자가 필요할 때 자동으로 서비스에 액세스
- **광범위한 네트워크 액세스**: 다양한 클라이언트 플랫폼을 통한 액세스
- **리소스 풀링**: 다중 테넌트 모델을 통한 리소스 공유
- **신속한 탄력성**: 수요에 따라 신속하게 확장/축소
- **측정 가능한 서비스**: 사용량 기반 과금

#### 클라우드 서비스 모델
- **IaaS (Infrastructure as a Service)**: 가상머신, 스토리지, 네트워크
- **PaaS (Platform as a Service)**: 개발 환경, 데이터베이스, 미들웨어
...
|------|-----|-----|
| **시장 점유율** | 1위 (32%) | 3위 (9%) |
| **주요 강점** | 서비스 다양성, 엔터프라이즈 | AI/ML, 데이터 분석 |
...
<details>
<summary>🔐 IAM 기초 이론</summary>

#### IAM (Identity and Access Management)이란?
IAM은 클라우드 리소스에 대한 접근을 안전하게 제어하는 서비스입니다.

#### AWS IAM 핵심 개념
- **사용자 (User)**: AWS 리소스에 접근하는 개인 또는 애플리케이션
- **그룹 (Group)**: 사용자들의 집합으로 권한을 그룹 단위로 관리
- **역할 (Role)**: AWS 서비스나 다른 AWS 계정이 사용할 수 있는 권한
- **정책 (Policy)**: 권한을 정의하는 JSON 문서

#### GCP IAM 핵심 개념
- **서비스 계정 (Service Account)**: 애플리케이션이 GCP 리소스에 접근할 때 사용
- **역할 (Role)**: 권한의 집합으로 미리 정의된 역할 사용
- **정책 (Policy)**: 누가 어떤 리소스에 대해 어떤 작업을 할 수 있는지 정의
...
<details>
<summary>💻 가상머신 서비스 기초 이론</summary>

#### 가상머신이란?
가상머신(VM)은 물리적 하드웨어를 소프트웨어로 추상화하여 여러 운영체제를 동시에 실행할 수 있게 하는 기술입니다.

#### AWS EC2 (Elastic Compute Cloud)
- **인스턴스 타입**: 범용, 컴퓨팅 최적화, 메모리 최적화, 스토리지 최적화
- **AMI (Amazon Machine Image)**: 인스턴스를 시작하는 데 필요한 정보가 포함된 템플릿
- **보안 그룹**: 인스턴스에 대한 가상 방화벽
- **키 페어**: 인스턴스에 안전하게 연결하기 위한 암호화된 키

#### GCP Compute Engine
- **머신 타입**: 범용, 메모리 최적화, 컴퓨팅 최적화, GPU
- **이미지**: 인스턴스를 생성하는 데 사용되는 운영체제 및 소프트웨어
- **방화벽 규칙**: 네트워크 트래픽을 제어하는 규칙
- **SSH 키**: 인스턴스에 안전하게 연결하기 위한 키
...
<details>
<summary>📦 스토리지 서비스 기초 이론</summary>

#### 클라우드 스토리지란?
클라우드 스토리지는 인터넷을 통해 데이터를 저장하고 접근할 수 있는 서비스입니다.

#### AWS S3 (Simple Storage Service)
- **버킷 (Bucket)**: 객체를 저장하는 컨테이너
- **객체 (Object)**: 버킷에 저장되는 파일과 메타데이터
- **키 (Key)**: 버킷 내 객체의 고유 식별자
...
| 클래스 | 접근 빈도 | 비용 | 용도 |
|--------|-----------|------|------|
| **Standard** | 자주 접근 | 높음 | 웹사이트, 모바일 앱 |
| **IA** | 가끔 접근 | 중간 | 백업, 재해 복구 |
| **Glacier** | 거의 접근 안함 | 낮음 | 장기 보관, 규정 준수 |
