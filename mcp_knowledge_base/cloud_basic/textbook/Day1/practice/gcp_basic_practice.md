...
[← 이전: AWS 기초 실습](./aws_basic_practice.md) | [📚 전체 커리큘럼](/curriculum.md) | [🏠 학습 경로로 돌아가기](/index.md) | [다음: 통합 실습 →](./실습1_aws_gcp.md) | [← 이전: Cloud Basic 메인](../README.md) | [📋 학습 경로](../learning-path.md)
...
- GCP 계정 생성 및 설정
- IAM 서비스 계정 및 권한 관리
- Compute Engine 인스턴스 생성 및 관리
- Cloud Storage 서비스 활용
...
# SSH 키 삭제
gcloud compute os-login ssh-keys remove --key-file=~/.ssh/gcp-student-key.pub
```
...
- [ ] Cloud Storage 버킷 생성 및 파일 업로드/다운로드
- [ ] 정적 웹사이트 호스팅 설정
- [ ] 리소스 정리 완료
...
3. **Cloud DNS**: DNS 관리
4. **App Engine**: 관리형 애플리케이션 플랫폼
5. **Cloud SQL**: 관리형 데이터베이스 서비스
