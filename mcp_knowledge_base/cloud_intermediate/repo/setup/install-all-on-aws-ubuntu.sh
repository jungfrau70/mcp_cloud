...
# MCP Cloud Intermediate - AWS Ubuntu 환경 전체 설치 스크립트
# 이 스크립트는 AWS Ubuntu 환경에서 모든 필요한 도구를 설치합니다.
...
if [[ -f /etc/os-release ]]; then
    . /etc/os-release
    if [[ "$ID" == "ubuntu" ]]; then
        log_success "Ubuntu 환경 확인됨: $VERSION"
    else
        log_warning "Ubuntu가 아닌 환경입니다: $ID"
...
# MCP Cloud Intermediate AWS 환경 설정
# 생성 시간: $(date)
...
if ! grep -q "MCP_CLOUD_HOME" ~/.bashrc; then
    echo "" >> ~/.bashrc
    echo "# MCP Cloud Intermediate AWS 환경 설정" >> ~/.bashrc
...
log_success "🎉 AWS Ubuntu 환경 설치가 완료되었습니다!"
