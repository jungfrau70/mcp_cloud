...
validate_environment() {
    ...
    if ! aws ecs describe-clusters --clusters "$ECS_CLUSTER_NAME" &> /dev/null; then
        log_warning "ECS 클러스터가 존재하지 않거나 접근할 수 없습니다: $ECS_CLUSTER_NAME"
    fi
...
# =============================================================================
create_cluster() {
    local cluster_name="${1:-ecs-intermediate}"
    ...
    log_info "태스크 패밀리: $TASK_FAMILY"
    log_info "로그 그룹: $LOG_GROUP"
    log_info "ALB 이름: $ALB_NAME"
    log_info "타겟 그룹: $TG_NAME"
    log_info "보안 그룹: $SECURITY_GROUP"
...
EOF
    ...
    # 태스크 정의 JSON을 임시 파일로 저장
    local temp_json_file="/tmp/task-definition-${task_family}.json"
    echo "$task_definition_json" > "$temp_json_file"
    
    # JSON 문법 검증
    if ! jq empty "$temp_json_file" 2>/dev/null; then
        log_error "JSON 문법 오류가 있습니다."
        rm -f "$temp_json_file"
        return 1
    fi
    ...
    if aws ecs register-task-definition --cli-input-json "file://$temp_json_file"; then
...
create_service() {
    ...
    # 같은 VPC에서 서브넷 및 보안 그룹 확인
    local subnet_id=$(aws ec2 describe-subnets --filters "Name=vpc-id,Values=$vpc_id" --query 'Subnets[0].SubnetId' --output text)
    local security_group_id=$(aws ec2 describe-security-groups --filters "Name=vpc-id,Values=$vpc_id" "Name=group-name,Values=default" --query 'SecurityGroups[0].GroupId' --output text)
