...
log_info "=== MCP Cloud Intermediate AWS Amazon Linux 환경 설치 시작 ==="
log_info "설치 시간: $(date)"
...
# 9. Python 3 및 pip 설치 (Amazon Linux용)
log_info "Python 3 및 pip 설치 중..."
if ! command -v python3 &> /dev/null; then
    # Amazon Linux 2에서는 python3가 기본 설치되어 있지만, pip가 없을 수 있음
...
# 15. Amazon Linux용 Docker 시작 스크립트 생성
log_info "Amazon Linux용 Docker 시작 스크립트 생성 중..."
cat > ~/.local/bin/start-docker << 'EOF'
#!/bin/bash
# AWS Amazon Linux에서 Docker 시작 스크립트

echo "🐳 AWS Amazon Linux에서 Docker 시작 중..."
...
# 16. Amazon Linux 환경 체크 스크립트 생성
log_info "Amazon Linux 환경 체크 스크립트 생성 중..."
cat > ~/.local/bin/check-amzn-env << 'EOF'
#!/bin/bash
# AWS Amazon Linux 환경 체크 스크립트

echo "🔍 AWS Amazon Linux 환경 체크 시작..."
...
echo "🎯 AWS Amazon Linux 환경 체크 완료!"
EOF

chmod +x ~/.local/bin/check-amzn-env
log_success "Amazon Linux 환경 체크 스크립트 생성 완료: ~/.local/bin/check-amzn-env"
...
# 18. Amazon Linux 특화 설정
log_info "Amazon Linux 특화 설정 중..."
...
if ! rpm -q yum-cron &> /dev/null; then
    ...
    log_success "yum-cron 설치 및 활성화 완료"
fi

# CloudWatch 에이전트 설치 (선택사항)
log_info "CloudWatch 에이전트 설치 여부 확인 중..."
if command -v amazon-cloudwatch-agent &> /dev/null; then
    log_info "CloudWatch 에이전트가 이미 설치되어 있습니다."
else
    log_warning "CloudWatch 에이전트가 설치되지 않았습니다."
    log_info "필요시 다음 명령으로 설치하세요:"
...
# 19. 설치 완료 메시지
log_success "=== MCP Cloud Intermediate AWS Amazon Linux 환경 설치 완료 ==="
...
log_info "환경 체크 스크립트: ~/.local/bin/check-amzn-env"
