...
# GCP GKE Monitoring Helper 모듈
# 역할: GCP GKE 클러스터 모니터링 관련 작업 실행 (클러스터 생성, 앱 배포, 모니터링 설정)
# 
# 사용법:
#   ./gcp-gke-monitoring-helper.sh --action <액션> [옵션]
...
옵션:
  --cluster-name <name> # GKE 클러스터 이름 (기본값: gke-monitoring-cluster)
  --node-type <type>    # 노드 머신 타입 (기본값: e2-medium)
  --node-count <count>  # 노드 개수 (기본값: 2)
  --region <region>     # GCP 리전 (기본값: 환경변수)
...
예시:
  $0 --action create-cluster
  $0 --action create-cluster --cluster-name my-cluster --node-type e2-standard-2
...
옵션:
  --cluster-name <name> # 삭제할 GKE 클러스터 이름
  --region <region>     # GCP 리전 (기본값: 환경변수)
  --zone <zone>         # GCP 존 (기본값: 환경변수)
  --force               # 확인 없이 강제 정리
...
옵션:
  --cluster-name <name> # 확인할 GKE 클러스터 이름
  --namespace <ns>      # Kubernetes 네임스페이스 (기본값: monitoring)
  --region <region>     # GCP 리전 (기본값: 환경변수)
  --zone <zone>         # GCP 존 (기본값: 환경변수)
...
validate_environment() {
    ...
    log_success "GCP GKE 모니터링 환경 검증 완료"
    return 0
}
...
# =============================================================================
create_cluster() {
    local cluster_name="${1:-gke-monitoring-cluster}"
    local node_type="${2:-e2-medium}"
    ...
    log_info "GKE 클러스터 생성 중... (클러스터: $cluster_name, 노드 타입: $node_type, 노드 수: $node_count)"
    update_progress "create-cluster" "started" "GKE 클러스터 생성 시작"
    ...
    if gcloud container clusters create "$cluster_name" \
        ...
        update_progress "create-cluster" "completed" "GKE 클러스터 생성 완료"
        ...
        # 클러스터 ID를 파일에 저장
        echo "$cluster_name" > "$LOGS_DIR/gke-cluster-name.txt"
        
        return 0
    else
        log_error "GKE 클러스터 생성 실패"
        update_progress "create-cluster" "failed" "GKE 클러스터 생성 실패"
        return 1
    fi
}
...
deploy_app() {
    ...
    if [ -z "$cluster_name" ]; then
        if [ -f "$LOGS_DIR/gke-cluster-name.txt" ]; then
            cluster_name=$(cat "$LOGS_DIR/gke-cluster-name.txt")
        else
...
cleanup_resources() {
    ...
    if gcloud container clusters delete "$cluster_name" --region="$region" --quiet; then
        ...
        # 클러스터 이름 파일 삭제
        rm -f "$LOGS_DIR/gke-cluster-name.txt"
...
main() {
    local action=""
    local cluster_name="gke-monitoring-cluster"
    local node_type="e2-medium"
    local node_count="2"
    ...
    case "$action" in
        "create-cluster")
            create_cluster "$cluster_name" "$node_type" "$node_count" "$region" "$zone"
            ;;
        "deploy-app")
            deploy_app "$cluster_name" "$namespace" "$region" "$zone"
            ;;
        "setup-monitoring")
            setup_monitoring "$cluster_name" "$namespace" "$region" "$zone"
            ;;
        "install-prometheus")
            install_prometheus "$cluster_name" "$namespace" "$region" "$zone"
            ;;
        "install-grafana")
            install_grafana "$cluster_name" "$namespace" "$region" "$zone"
            ;;
        "setup-alerts")
            setup_alerts "$cluster_name" "$namespace" "$region" "$zone"
            ;;
        "cleanup")
            cleanup_resources "$cluster_name" "$region" "$zone" "$force"
            ;;
        "status")
            check_cluster_status "$cluster_name" "$namespace" "$region" "$zone" "$format"
            ;;
        *)
