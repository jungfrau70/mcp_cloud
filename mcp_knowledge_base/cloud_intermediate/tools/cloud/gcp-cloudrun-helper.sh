...
manage_traffic() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # 트래픽 상태 확인
        log_info "트래픽 상태 확인:"
        gcloud run services describe "$service_name" --region="$region" --format="table(metadata.name,status.traffic[].revisionName,status.traffic[].percent)"
...
check_cloudrun_status() {
    ...
    case "$format" in
        "json")
            ...
            ;;
        "yaml")
            gcloud run services list --region="$region" --format="yaml"
            ;;
        *)
            gcloud run services list --region="$region" --format="table(metadata.name,status.url,spec.template.spec.containers[0].image,status.conditions[0].status)"
            ;;
    esac
    
    # 특정 서비스 상세 정보
    if [ -n "$service_name" ]; then
        log_info "서비스 상세 정보: $service_name"
        case "$format" in
            "json")
            ...
            "yaml")
                ...
                ;;
            *)
                gcloud run services describe "$service_name" --region="$region" --format="table(metadata.name,status.url,spec.template.spec.containers[0].image,status.conditions[0].status,status.traffic[].revisionName,status.traffic[].percent)"
                ;;
        esac
    fi
