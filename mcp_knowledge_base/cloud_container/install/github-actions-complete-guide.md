...
## 📖 현재 위치
**Cloud Container** > **1일차** > **GitHub Actions 완전 가이드**
...
#### GitHub에 공개 키 등록
1. 공개 키 복사: `cat ~/.ssh/id_ed25519.pub`
...
#### 1. **워크플로우 파일 관리**
- 워크플로우 파일은 작고 집중적으로 유지
- 재사용 가능한 워크플로우는 별도 파일로 분리
- 의미 있는 이름과 설명 사용
...
# ❌ 잘못된 예시
- name: Bad practice
  run: echo "API_KEY=${{ secrets.API_KEY }}" >> $GITHUB_OUTPUT
...
jobs:
  ...
  build:
    needs: test  # build가 test 결과에 의존하지 않음
    runs-on: ubuntu-latest
    steps: [...]
