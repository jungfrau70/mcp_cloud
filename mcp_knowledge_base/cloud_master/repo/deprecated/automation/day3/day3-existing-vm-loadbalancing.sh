...
# Cloud Master Day3 - 기존 VM 활용 로드밸런싱 실습
# 작성일: 2024년 9월 23일
# 목적: 기존 cloud-deployment-server VM들을 활용한 로드밸런싱 설정
...
# 설정 변수
PROJECT_NAME="cloud-master-day3-existing-vm"
...
GCP_INSTANCE_NAME="cloud-deployment-server"
...
verify_existing_vms() {
    ...
    # GCP VM 확인
    log_info "GCP VM 확인 중..."
    local gcp_vm_info
    gcp_vm_info=$(run_with_timeout 30 gcloud compute instances describe "$GCP_INSTANCE_NAME" \
        --zone=asia-northeast3-a \
        --format="value(name,status,networkInterfaces[0].accessConfigs[0].natIP,networkInterfaces[0].networkIP)")
    
    if [ $? -ne 0 ] || [ -z "$gcp_vm_info" ]; then
        log_error "GCP VM을 찾을 수 없습니다: $GCP_INSTANCE_NAME"
        return 1
    fi
    
    read -r gcp_name gcp_status gcp_public_ip gcp_private_ip <<< "$gcp_vm_info"
    
    if [ "$gcp_status" != "RUNNING" ]; then
        log_error "GCP VM이 실행 중이 아닙니다: $gcp_status"
        return 1
    fi
    
    log_success "GCP VM 확인 완료"
    log_info "  - Instance Name: $gcp_name"
    log_info "  - Public IP: $gcp_public_ip"
    log_info "  - Private IP: $gcp_private_ip"
...
GCP_INSTANCE_NAME=$gcp_name
GCP_PUBLIC_IP=$gcp_public_ip
GCP_PRIVATE_IP=$gcp_private_ip
EOF
}

setup_aws_load_balancer() {
    log_step "2/6: AWS 로드밸런서 설정"
    ...
    # 서브넷 ID 가져오기
    local subnet_id
    subnet_id=$(run_with_timeout 30 aws ec2 describe-instances \
        --instance-ids "$AWS_INSTANCE_ID" \
        --query 'Reservations[0].Instances[0].SubnetId' --output text)
    
    log_info "서브넷 ID: $subnet_id"
...
setup_gcp_load_balancer() {
    ...
    run_with_timeout 30 gcloud compute instance-groups unmanaged create "$PROJECT_NAME-instance-group" \
        --zone=asia-northeast3-a &>/dev/null
    ...
    run_with_timeout 30 gcloud compute instance-groups unmanaged add-instances "$PROJECT_NAME-instance-group" \
        --instances="$GCP_INSTANCE_NAME" --zone=asia-northeast3-a &>/dev/null
    ...
    run_with_timeout 30 gcloud compute backend-services add-backend "$PROJECT_NAME-backend-service" \
        ...
        --instance-group-zone=asia-northeast3-a --global &>/dev/null
...
# Cloud Master Day3 - 기존 VM 활용 비용 최적화 리포트
...
show_status() {
    ...
    if [ -f "vm-info.env" ]; then
        ...
        echo "  GCP: $GCP_INSTANCE_NAME ($GCP_PUBLIC_IP)"
    fi
...
cleanup() {
    ...
    if [ -f "gcp-lb-config.env" ]; then
        ...
        run_with_timeout 30 gcloud compute instance-groups unmanaged delete "$PROJECT_NAME-instance-group" --zone=asia-northeast3-a --quiet &>/dev/null
...
show_help() {
    echo "Cloud Master Day3 - 기존 VM 활용 로드밸런싱 실습"
    echo ""
    echo "사용법: $0 [옵션]"
    echo ""
    echo "옵션:"
    ...
    echo ""
    echo "특징:"
    echo "  - 기존 cloud-deployment-server VM 활용"
    echo "  - AWS/GCP 로드밸런서 설정"
    echo "  - 통합 모니터링 설정"
    echo "  - 비용 최적화 분석"
}
...
main() {
    case "${1:-setup}" in
        "setup")
            ...
            test_system
            show_status
            log_success "Cloud Master Day3 기존 VM 활용 실습 완료!"
            ;;
        "test")
            test_system
            ;;
