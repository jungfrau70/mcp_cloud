...
[🛠️ 실습 학습](#실습-학습)
6. [🔧 실습 환경 준비](#실습-환경-준비)
7. [🐳 Docker 기초 및 컨테이너 기술 실습](#docker-설치-및-설정-컨테이너-오케스트레이션-구현)
8. [📝 Git/GitHub 기초 및 협업 실습](#kubernetes-기초-및-클러스터-관리)
9. [🚀 GitHub Actions CI/CD 파이프라인 실습](#github-actions-고급-워크플로우)
10. [🖥️ VM 기반 웹 애플리케이션 배포 실습](#cloud-master-2일차-고급-cicd-및-vm-기반-컨테이너-배포-이론-및-실습)
...
[🐳 Docker 고급 기법 및 최적화 이론](#docker-고급-기법-및-최적화-이론)
...
[Docker 이미지 최적화 기법](#docker-이미지-최적화-기법)
...
[Docker Compose 고급 기능](#docker-compose-고급-기능)
...
[VM 기반 컨테이너 배포란?](#vm-기반-컨테이너-배포란)
가상머신에 컨테이너를 배포하여 고가용성과 확장성을 확보하는 방식입니다.

#### VM 기반 배포 장점

[VM 기반 배포 장점](#vm-기반-배포-장점)
...
[컨테이너 오케스트레이션 도구](#컨테이너-오케스트레이션-도구)
...
[📦 필수 소프트웨어 설치](#필수-소프트웨어-설치)

#### 1. AWS CLI 설치 및 설정

[1. AWS CLI 설치 및 설정](#1-aws-cli-설치-및-설정)
```bash
# AWS CLI 설치 확인
aws --version
...
[2. Google Cloud SDK 설치 및 설정](#2-google-cloud-sdk-설치-및-설정)
```bash
# gcloud CLI 설치 확인
gcloud --version
...
[3. 컨테이너 관리 도구 설치](#3-컨테이너-관리-도구-설치)
```bash
...
[Google Cloud Platform 계정 설정](#google-cloud-platform-계정-설정)
- [ ] GCP 계정 생성 ($300 크레딧)
- [ ] Billing 계정 설정
...
[🐳 Docker 고급 기법 및 최적화](#docker-고급-기법-및-최적화)

### 📚 이론: 컨테이너 아키텍처 원리

[📚 이론: 컨테이너 아키텍처 원리](#이론-컨테이너-아키텍처-원리)

#### 컨테이너 기술의 핵심 개념

[컨테이너 기술의 핵심 개념](#컨테이너-기술의-핵심-개념)
...
[🛠️ 실습: 프로덕션급 Docker 이미지 빌드](#실습-프로덕션급-docker-이미지-빌드)
1. **기존 Dockerfile 분석**
2. **멀티스테이지 빌드 적용**
3. **이미지 크기 최적화**
4. **보안 취약점 스캔**
...
[📚 이론: CI/CD 파이프라인 아키텍처](#이론-cicd-파이프라인-아키텍처)

#### CI/CD의 핵심 원리

[CI/CD의 핵심 원리](#cicd의-핵심-원리)
...
[GitHub Actions 아키텍처](#github-actions-아키텍처)
...
[실습: 고급 CI/CD 파이프라인 구축](#실습-고급-cicd-파이프라인-구축)
1. **매트릭스 빌드 설정**
2. **환경별 배포 자동화**
3. **시크릿 관리 구현**
4. **성능 최적화**
...
[📚 이론: Kubernetes 기초 개념](#이론-kubernetes-기초-개념)
...
[Kubernetes 기본 구성 요소](#kubernetes-기본-구성-요소)
...
[EKS 클러스터 생성 (AWS)](#eks-클러스터-생성-aws)
```bash
# EKS 클러스터 생성
...
[GKE 클러스터 생성 (GCP)](#gke-클러스터-생성-gcp)
```bash
# GKE 클러스터 생성
...
[ConfigMap과 Secret 관리](#configmap과-secret-관리)
```bash
# ConfigMap 생성
...
[실습: Kubernetes 기초 배포](#실습-kubernetes-기초-배포)
1. **EKS/GKE 클러스터 생성**
2. **기본 애플리케이션 배포**
3. **서비스 노출 및 접근**
4. **스케일링 및 업데이트**
...
[🚀 VM 기반 컨테이너 배포 자동화](#vm-기반-컨테이너-배포-자동화)

### 📚 이론: 클라우드 인프라 아키텍처

[📚 이론: 클라우드 인프라 아키텍처](#이론-클라우드-인프라-아키텍처)

#### 가상화 기술의 원리

[가상화 기술의 원리](#가상화-기술의-원리)
...
[VM 인스턴스 생성 (이론 적용)](#vm-인스턴스-생성-이론-적용)
```bash
...
[실습: VM 기반 컨테이너 배포](#실습-vm-기반-컨테이너-배포)
1. **VM 인스턴스 생성**
2. **Docker 환경 구축**
3. **컨테이너 배포**
4. **고가용성 설정**
...
[🔄 완전 자동화된 배포 파이프라인](#완전-자동화된-배포-파이프라인)

### 📚 이론: DevOps 및 자동화 원리

[📚 이론: DevOps 및 자동화 원리](#이론-devops-및-자동화-원리)

#### DevOps 문화와 원칙

[DevOps 문화와 원칙](#devops-문화와-원칙)
...
[자동화 파이프라인의 핵심 요소](#자동화-파이프라인의-핵심-요소)
...
[GitHub Actions 워크플로우 (DevOps 원리 적용)](#github-actions-워크플로우-devops-원리-적용)
```yaml
...
[실습: 완전 자동화 파이프라인 구축 (DevOps 원리 적용)](#실습-완전-자동화-파이프라인-구축-devops-원리-적용)
...
[💡 핵심 개념 미리보기](#핵심-개념-미리보기)

### 클라우드 비용 구조

[클라우드 비용 구조](#클라우드-비용-구조)
