...
[📋 목차](#-)
1. [모니터링 개념 이해](#-)
2. [AWS CloudWatch vs GCP Cloud Monitoring 비교](#-aws-cloudwatch-vs-gcp-cloud-monitoring)
3. [모니터링 아키텍처](#-)
4. [메트릭 및 로그 관리](#-)
5. [알림 및 대시보드](#-)
6. [실습 목표](#-)
...
[AWS CloudWatch 메트릭](#-aws-cloudwatch)
...
[GCP Cloud Monitoring 메트릭](#-gcp-cloud-monitoring)
...
[AWS CloudWatch 아키텍처](#-aws-cloudwatch)
```mermaid
graph TB
...
[GCP Cloud Monitoring 아키텍처](#-gcp-cloud-monitoring)
```mermaid
graph TB
...
[AWS CloudWatch 알람](#-aws-cloudwatch)
```bash
# CPU 사용률 알람 생성
...
[GCP Cloud Monitoring 알림](#-gcp-cloud-monitoring)
```bash
# 알림 정책 생성
...
[AWS CloudWatch 대시보드](#-aws-cloudwatch)
```bash
# 대시보드 생성
...
[GCP Cloud Monitoring 대시보드](#-gcp-cloud-monitoring)
```bash
# 대시보드 생성
...
[AWS CloudWatch Agent 설치](#-aws-cloudwatch-agent)
```bash
...
[GCP Cloud Monitoring Agent 설치](#-gcp-cloud-monitoring-agent)
```bash
...
[AWS CloudWatch 메트릭 확인](#-aws-cloudwatch)
```bash
# EC2 인스턴스 메트릭 조회
...
[GCP Cloud Monitoring 메트릭 확인](#-gcp-cloud-monitoring)
```bash
# VM 인스턴스 메트릭 조회
...
[AWS CloudWatch 알람 설정](#-aws-cloudwatch)
```bash
# SNS 토픽 생성
...
[GCP Cloud Monitoring 알림 설정](#-gcp-cloud-monitoring)
```bash
# 알림 채널 생성
gcloud monitoring channels create \
  --display-name="Email Alert" \
...
다음 단계: [4교시: 종합 실습 - 컨테이너 자동 배포 + 로드밸런싱 + 오토스케일링](/mcp_knowledge_base/cloud_master/textbook/Day2/comprehensive-practice-guide.md)
