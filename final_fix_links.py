#!/usr/bin/env python3
"""
최종 링크 수정 스크립트 - 상대 경로 링크들을 절대 경로로 변환
"""
import os
import re
import json
...
class FinalLinkFixer:
    ...
    def resolve_relative_path(self, current_file: Path, target_url: str) -> Optional[str]:
        """상대 경로를 절대 경로로 변환"""
    ...
    def fix_links_in_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            def replace_markdown_link(match):
                ...
                # 대상 파일 찾기
                target_file = self.resolve_relative_path(file_path, url)
                if target_file:
