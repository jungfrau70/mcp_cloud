#!/usr/bin/env python3
"""
Cloud Basic textbook 디렉토리의 모든 마크다운 파일 검사
"""

import re
from pathlib import Path

def check_cloud_basic_files():
    """Cloud Basic textbook 디렉토리의 모든 마크다운 파일 검사"""
    
    base_path = Path("mcp_knowledge_base/cloud_basic/textbook")
    
    if not base_path.exists():
        print("❌ Cloud Basic textbook 디렉토리를 찾을 수 없습니다.")
        return
    
    print("🔍 Cloud Basic textbook 디렉토리 검사 시작...\n")
...
def check_markdown_file(content, file_path):
    ...
    for level, title in headings:
        ...
        if re.search(r'[🎯📚🔧🚀🐳☸️🖥️💾🔐☁️🏗️📊🔄💰]', title):
            ...
            if not found_anchor:
                issues.append({
                    'file': str(file_path.relative_to(Path("mcp_knowledge_base/cloud_basic/textbook"))),
                    'type': 'missing_anchor',
